...
char infixExp[MAX_EXPRESSION_SIZE]; //크기가 20인 문자형 배열 infixExp 선언
char postfixExp[MAX_EXPRESSION_SIZE]; //크기가 20인 문자형 배열 postfixExp 선언
char postfixStack[MAX_STACK_SIZE]; //크기가 10인 문자형 배열 postfixStack 선언
int evalStack[MAX_STACK_SIZE]; //크기가 10인 정수형 배열 evalStack 선언

int postfixStackTop = -1;  // postfixStackTop을 -1로 초기화
int evalStackTop = -1; // evalStackTop을 -1로 초기화

int evalResult = 0; // evalResult을 0으로 초기화 
...
int main()
{
	...
	do{
		...
		case 'i': case 'I': //i 일 경우
			getInfix();//getInfix함수 호출
			break;
		case 'p': case 'P': //p 일 경우
			toPostfix(); //toPostfix 함수 호출
			break;
		case 'e': case 'E': //e 일 경우
			evaluation(); //evaluation 함수 호출
			break;
		case 'd': case 'D'://d 일 경우
			debug(); //debug 함수 호출
			break;
		case 'r': case 'R': //r 일 경우
			reset(); //reset 함수 호출
			break;
		case 'q': case 'Q': //q 일 경우
			break;
	...
	}while(command != 'q' && command != 'Q'); //q ,Q 가 입력될 때까지 반복.
...
{
    postfixStack[++postfixStackTop] = x; //x 값을 증가시킨 postfixStackTop 위치에 저장
}
...
char postfixPop() //스택에서 Pop하는 함수 
{
    ...
    if(postfixStackTop == -1) //postfixStackTop 이 -1 일 경우 즉, postfixStackTop이 공백스택일 때
    	...
    	x = postfixStack[postfixStackTop--]; // x값을 감소시킨 postfixStackTop 위치에 저장
...
{
    if(evalStackTop == -1) //evalStackTop이 -1이면 evalStackTop이 공백스택일때를 말함
        return -1;
    else
